...
export default function SavannahGame({ words, setStart }: Props) {
  ...
  useEffect(() => {
    if (
      state.click &&
      words &&
      state.SuccessWords.length + state.FailWords.length !== words.length - 4 &&
    ...
    <>
      ...
      <div className="savannah_progress">
        {words ? (
          <ProgressBar
            data={[
              state.SuccessWords.length,
              state.FailWords.length,
              words.length - 4,
            ]}
          />
        ) : (
      ...
      <div className="choose_word_btn">
        {state.wordsBtns.length !== 0 && words ? (
          state.wordsBtns.map((e: any, i: any) => {
            return (
              <Button
                style={
                  state.click
                    ? e["wordTranslate"] === state.word["wordTranslate"]
                      ? { background: "#52c41a" }
                      : { background: "#f61c1c" }
                ...
                key={i}
                onClick={() => handlerClick(e["wordTranslate"])}
              >{`${i + 1}.${e["wordTranslate"]}`}</Button>
            );
          })
        ) : (
      ...
      <div className="savannah_icon">
        <img
          className="savannah_icon_tree"
          style={state.click ? { animation: "tree 0.8s infinite linear" } : {}}
          src="../../img/tree.svg"
          alt="tree"
        />
