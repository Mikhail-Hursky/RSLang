  ...
  return (
    <>
      <div className="sprint_container"> 
        ...
        {state.wordsBtns.length !== 0 && words && !state.click ? <SprintWord timer={fail} word={[state.word['word'], state.wordsBtns['wordTranslate']]} click={state.click}/> : <Spin style={{height: '120px'}} size="large" />}
        <div className="sprint_contant_center">
        <img src="../../img/sprint.png" alt="sprint"/>
  
        {state.wordsBtns.length !== 0 && words ? <SprintTimer timeout={setTime} /> : <Spin style={{height: '120px'}} size="large" />}
        </div>
        
        <div className="choose_sprint_btn">
       <Button style={{background: "#e00303"}} autoFocus={!state.click} disabled={state.click} onClick={() => handlerClick(false)} ><CaretLeftOutlined /> Неверно</Button>  
       <Button style={{background: "#11e003"}} autoFocus={!state.click} disabled={state.click} onClick={() => handlerClick(true)} >Верно <CaretRightOutlined /></Button>  
      </div> 
      </div>
      <FullscreenGame />
      {(words && state.SuccessWords.length + state.FailWords.length === words.length) || state.FailWords.length === 5 || time ? 
