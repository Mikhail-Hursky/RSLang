...
export default function AudioCallGame({ words, setStart }: Props) {
  ...
  const [stat, setStat] = useState<any>({successWords:[], failWords:[]});
  ...
  const handleClick = () => {
    if (!isReplied) {
      setIsReplied(true);
      setStat({...stat, failWords: [...stat.failWords, words[index]]});
